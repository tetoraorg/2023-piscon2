...
func getIsuGraph(c echo.Context) error {
	...
	_, err = isuCache.Get(c.Request().Context(), jiaUserID+"*"+jiaIsuUUID)
	if errors.Is(err, sql.ErrNoRows) {
		return c.String(http.StatusNotFound, "not found: isu")
	}
	if err != nil {
	...
	res, err := graphResponseCache.Get(c.Request().Context(), jiaIsuUUID+"*"+date.Format("2006-01-02T15:04:05Z07:00"))
	if err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func generateIsuGraphResponse(_ context.Context, jiaIsuUUIDWithGraphDate string) ([]GraphResponse, error) {
	...
	graphDate, err := time.Parse("2006-01-02T15:04:05Z07:00", graphDateStr)
	if err != nil {
...
func getIsuConditions(c echo.Context) error {
	...
	conditionsResponse, err := getIsuConditionsFromDB(db, jiaIsuUUID, endTime, conditionLevel, startTime, conditionLimit, isu.Name)
	if err != nil {
		c.Logger().Errorf("db error: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
